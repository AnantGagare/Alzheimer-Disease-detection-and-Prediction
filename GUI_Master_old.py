...
#
lbl = tk.Label(root, text="Alzheimer Disease detection", font=('times', 35,' bold '), width=50,height=1,bg="Red3",fg="white")
lbl.place(x=0, y=0)
...
#    global fn
    ...
    if fn!="":
        ...
        alzheimer=np.argmax(prediction)
        print(alzheimer)
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:\Alzheimer_Disease', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
